...
function selectinput() {
    var value =jQuery("#semester").val();
    var periodID = jQuery("#periodid").val();
    if (periodID == 0) {
	last_valid_selection = value;
	if(value == "Other") {
	    jQuery("#newSemester").css("display","inline");
	}
	else {
	    jQuery("#newSemester").css("display","none");
	}
    }else {
	alert("Editing" + last_valid_selection);
	jQuery("#semester").val(last_valid_selection);
    }
}
//edit function
...
jQuery(function ($) {
	...
	if(semester == "Other" || periodID != 0) {
	    semester = $('#newSemester').val();
	    if(semester=="") {
		alert (periodStrings.semesterVal);
		return false;   
	    }
	...
	$.get('admin-ajax.php' , data, function(data){
		...
		$('#newSemester').val("");
		$('#newSemester').css("display", "none");
		isDirty = 0;
		//Load the options again
		loadSemesters();
		//fail
	    }else{
		//insert
...
function loadSemesters(){
    //ajax data
    data = {
	action: 'utt_load_periodsemesters'
    };
    //ajax call
    jQuery.get('admin-ajax.php', data, function(data){
	//load new data
	jQuery('#semester').html(data);
    });
}