...
?>
	...
	<form action="" name="periodForm" method="post">
	    ...
	    <input type="number" name="year" id="year" class="dirty" value="<?php echo date("Y"); ?>"/>
            ...
            <select name="semester" class="dirty" id="semester" onchange="selectinput()">
		<option value=0><?php _e("- select -","UniTimetable"); ?></option>
		<?php
		global $wpdb;
		$periodsTable=$wpdb->prefix."utt_periods";
		$periods = $wpdb->get_results("SELECT * FROM $periodsTable ORDER BY semester ASC");
		foreach($periods as $period) {
		    echo "<option value=" . $period->semester . '>' ;echo "$period->semester"; echo "</option>";
		}
		?>
		<option value="Other"><?php _e("Other","UniTimetable"); ?></option>		
            </select>
	    <input type="text" id="newSemester" name="newSemester" value="" placeholder="Enter Semester" />
            <br/>
...
?>
    ...
    add_action('wp_ajax_utt_load_periodsemesters','utt_load_periodsemesters');
    function utt_load_periodsemesters(){
 	//semester number selected
 	global $wpdb;
 	$periodsTable = $wpdb->prefix."utt_periods";
 	$safeSql = $wpdb->prepare("SELECT * FROM $periodsTable ORDER BY semester ASC");
 	$periods = $wpdb->get_results($safeSql);
	echo "<option value=0>";_e("- select -","UniTimetable");echo "</option>";
	foreach($periods as $period){
            //if edit, select the stored period
	    echo "<option value='$period->semester'>$period->semester</option>";
	}
